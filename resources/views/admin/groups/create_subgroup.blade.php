...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">@lang('main.group.create_subgroup')</li>
    </ol>
    <div class="card mb-3">
        <div class="card-header">
            @lang('main.group.create_subgroup')
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <form action="{{route('subgroup.store')}}" method="POST" enctype="multipart/form-data">
                    {{csrf_field()}}
                    ...
                    <div class="form-group">
                        ...
                        <select name="parent" id="" class="form-control">
                            @foreach($parent_groups as $id=>$group)
                                <option {{($group_id == $id)? 'selected':''}} value="{{$id}}">{{$group['title']}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <div class="text-center">
                            <button class="btn btn-dark" type="submit">@lang('main.group.create_subgroup')</button>
                        </div>
                    </div>
                </form>
            </div>
