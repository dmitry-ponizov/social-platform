...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">@lang('main.rule.edit')</li>
    </ol>
    <div class="card mb-3">
        <div class="card-header">
            <div>
                @lang('main.rule.edit'): {{$rule->locale}}
            </div>
            ...
            <div class="table-responsive">
                <form action="{{route('rule.update',$rule->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input name="label" type="text" value="{{$rule->label}}" class="form-control">
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" class="form-control">
                            @foreach($types as $key=>$type)
                                <option {{($rule->type == $key)?'selected':''}} value="{{$key}}">{{$type}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input name="lang[ru]" type="text" value="{{$rule->lang->ru}}" class="form-control">
                        <span class="text-danger">{{$errors->first('lang.ru')}}</span>
                    </div>
                    <div class="form-group">
                        <label for="name">Ua</label>
                        <input name="lang[ua]" type="text" value="{{$rule->lang->ua}}" class="form-control">
                        <span class="text-danger">{{$errors->first('lang.ua')}}</span>
                    </div>
                    <div class="form-group">
                        <label for="name">En</label>
                        <input name="lang[en]" type="text" value="{{$rule->lang->en}}" class="form-control">
                        <span class="text-danger">{{$errors->first('lang.en')}}</span>
                    </div>
                    <div class="form-group">
                        <div class="text-center">
                            <button class="btn btn-dark" type="submit">@lang('main.rule.update')</button>
                        </div>
                    </div>
                </form>
            </div>
